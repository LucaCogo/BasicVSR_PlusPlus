...
@PIPELINES.register_module()
class GenerateSegmentIndicesPrecomp:
    """Generate frame indices for a segment. It also performs temporal
    augmention with random interval.
    ...
    def __call__(self, results):
        ...
        lq_path_root = results['lq_path']
        gt_path_root = results['gt_path']
        of_b_path_root = results['of_b_path']
        of_f_path_root = results['of_f_path']
        ...
        of_b_path = [
            osp.join(of_b_path_root, clip_name, '{:08d}'.format(v))
            for v in neighbor_list
        ]
        of_f_path = [
            osp.join(of_f_path_root, clip_name, '{:08d}'.format(v))
            for v in neighbor_list
        ]

        results['lq_path'] = lq_path
        results['gt_path'] = gt_path
        results['of_b_path'] = of_b_path
        results['of_f_path'] = of_f_path
        results['interval'] = interval
